 ...
 *
 */
if (!class_exists('WPA_Fields')) {
    abstract class WPA_Fields
    {
        public $field         = array();
        public $value         = '';
        ...
        public function field_after()
        {
            ...
            $outputput .= (!empty($this->field['help'])) ? '<span class="wpa-help"><span class="wpa-help-text">'.$this->field['help'].'</span><span class="fa fa-question-circle"></span></span>' : '';
            $outputput .= (!empty($this->field['_error'])) ? '<p class="wpa-text-error">'.$this->field['_error'].'</p>' : '';
        ...
        {
            $output = '';
            if (!empty($this->where) && (!empty($this->field['debug']) || (defined('WPA_DEBUG') && WPA_DEBUG))) {
                $value  = $this->field_value();
                $output .= "<pre>";
                $output .= "<strong>".esc_html__('CONFIG', 'nutrail').":</strong>";
                $output .= "\n";
                ob_start();
                ...
                if (!empty($this->field['id'])) {
                    $output .= "\n";
                    $output .= "<strong>".esc_html__('ID', 'nutrail').":</strong>";
                    $output .= "\n";
                    $output .= $this->field['id'];
                    $output .= "\n";
                    $output .= "\n";
                    $output .= "<strong>".esc_html__('USAGE', 'nutrail').":</strong>";
                    $output .= "\n";
                    ...
                    if (isset($value)) {
                        $output .= "\n\n";
                        $output .= "<strong>".esc_html__('VALUE', 'nutrail').":</strong>";
                        $output .= "\n";
                        ob_start();
                        var_export($value);
                        $output .= htmlspecialchars(ob_get_clean());
            ...
            if (!empty($this->where) && !empty($this->field['id']) && (!empty($this->field['debug_light']) || (defined('WPA_DEBUG_LIGHT') && WPA_DEBUG_LIGHT))) {
                $output .= "<pre>";
                $output .= "\n";
                $output .= "<strong>".esc_html__('ID', 'nutrail').":</strong>";
                $output .= "\n";
        ...
        {
            return (!empty($this->field['multilang'])) ? wpa_language_defaults() : false;
        }
            ...
            if (is_array($this->multilang)) {
                ...
                foreach ($this->multilang['languages'] as $key => $val) {
                    ...
                    if ($key != $this->multilang['current']) {
                        ...
                        // set default value
                        if (isset($this->default_value[$key])) {
                            $value = $this->default_value[$key];
                        } elseif (!isset($this->default_value[$key]) && ($key == $this->multilang['default'])) {
                        ...
                        if (in_array($this->field['type'], $simple_value)) {
                            ...
                            $output .= WPA::field($cache_field, $value, $this->unique);
                        }
                    }
                }
                $output .= '<input type="hidden" name="'.$this->field_name('[multilang]', true).'" value="true" />';
                $output .= '</fieldset>';
                $output .= '<p class="wpa-text-desc">'.sprintf('%s ( <strong>%s</strong> )', esc_html__('You are editing language:', 'nutrail'), $this->multilang['current']).'</p>';
            }

            return $output;
        }

        public static function getCategoryChildsFull($parent_id, $array, $level, $return_id, &$dropdown)
        {
            ...
            while ($i < count($array)) {
                ...
                if ($item->parent == $parent_id) {
                    ...
                    unset($array[$key]);
                    $array = self::getCategoryChildsFull($item->term_id, $array, $level + 1, $return_id, $dropdown);
                    $keys  = array_keys($array);
                    $i     = 0;
                }
            }
            ...
            switch ($type) {
                ...
                case 'menus':
                    ...
                    if (!is_wp_error($query) && !empty($query->terms)) {
                        $return_id = !empty($query_args['data-slug']) ? false : true;
                        self::getCategoryChildsFull(0, $query->terms, 0, $return_id, $options);
                        /*
                        foreach ($query->terms as $item) {
                            $value           = !empty($query_args['data-slug']) ? $item->slug : $item->term_id;
                            $options[$value] = $item->name.' ('.$item->count.')';
                        }
                        */
                    }

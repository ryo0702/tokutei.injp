...
class WPA_Breadcrumb
{
    ...
    {
        $args = wp_parse_args($args, apply_filters('wpa_breadcrumb_defaults',
            array(
                'delimiter'   => '',
                'wrap_before' => '',
                'wrap_after'  => '',
        ...
        $breadcrumbs = new WPA_Breadcrumb();

        if (!empty($args['home'])) {
            $breadcrumbs->add_crumb($args['home'], apply_filters('wpa_breadcrumb_home_url', home_url()));
        }
    ...
    {
        $this->add_crumb(single_post_title('', false) ?? 'Home');
    }
    ...
    {
        $this->add_crumb(__('Error 404', 'nutrail'));
    }
    ...
    protected function add_crumbs_product_tag()
    {
        ...
        $this->add_crumb(sprintf(__('Products tagged &ldquo;%s&rdquo;', 'nutrail'), $current_term->name), get_term_link($current_term, 'product_tag'));
    }
    ...
    protected function add_crumbs_tag()
    {
        ...
        $this->add_crumb(sprintf(__('Posts tagged &ldquo;%s&rdquo;', 'nutrail'), single_tag_title('', false)), get_tag_link($queried_object->term_id));
    }
    ...
    protected function add_crumbs_author()
    {
        ...
        $this->add_crumb(sprintf(__('Author: %s', 'nutrail'), $userdata->display_name));
    }
    ...
    protected function search_trail()
    {
        if (is_search()) {
            ...
            $this->add_crumb(sprintf(__('Search results for &ldquo;%s&rdquo;', 'nutrail'), get_search_query()), remove_query_arg('paged'));
        }
    }
    ...
    protected function paged_trail()
    {
        if (get_query_var('paged')) {
            ...
            $this->add_crumb(sprintf(__('Page %d', 'nutrail'), get_query_var('paged')));
        }
    }
}

 ...
 *
 */
if (!class_exists('WPA_Taxonomy')) {
    class WPA_Taxonomy extends WPA_Abstract
    {
        // constants
        ...
        public function __construct($sections)
        {
            ...
            $this->sections = apply_filters('wpa_options_taxonomy', $sections);
        ...
        public function add_taxonomy_fields()
        {
            foreach ($this->sections as $option) {
                ...
                $get_taxonomy = wpa_get_var('taxonomy');
        ...
        public function render_taxonomy_form_fields($term)
        {
            ...
            wp_nonce_field('wpa-taxonomy', 'wpa-taxonomy-nonce');

            do_action('wpa_html_taxonomy_before');

            echo '<div class="wpa wpa-taxonomy wpa-taxonomy-'.$classname.'-fields wpa-onload">';
            ...
            foreach ($this->sections as $option) {
                if ($taxonomy == $option['taxonomy']) {
                    if ($form_edit) {
                        ...
                        $timein       = wpa_timeout($timenow, $expires, 30);
                    ...
                    foreach ($option['fields'] as $field) {
                        ...
                        echo WPA::field($field, $elem_value, $option['id'], 'taxonomy');
                    }
                }
            }
            ...
            do_action('wpa_html_taxonomy_after');
        }

        // save taxonomy form fields
        public function save_taxonomy($term_id)
        {
            if (wp_verify_nonce(wpa_get_var('wpa-taxonomy-nonce'), 'wpa-taxonomy')) {
                $errors   = array();
                $taxonomy = wpa_get_var('taxonomy');
                foreach ($this->sections as $request_value) {
                    if ($taxonomy == $request_value['taxonomy']) {
                        ...
                        $request = apply_filters('wpa_save_taxonomy', $request, $request_key, $term_id);
                        if (empty($request)) {
                ...
                set_transient('wpa-taxonomy-transient', $errors, 10);
            }
        }

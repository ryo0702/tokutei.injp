 ...
 *
 */
if (!class_exists('WPA_Profile')) {
    class WPA_Profile extends WPA_Abstract
    {
        // constants
        ...
        {
            // Get options profile
            $this->sections = apply_filters('wpa_addon_options_profile', $sections);
        ...
        public function render_profile_form_fields($user)
        {
            ...
            wp_nonce_field('wpa-profile', 'wpa-profile-nonce');

            do_action('wpa_html_profile_before');

            echo '<div class="wpa wpa-profile wpa-profile-'.$classname.'-fields wpa-onload">';
            ...
            foreach ($this->sections as $option) {
                ...
                foreach ($option['fields'] as $field) {
                    ...
                    echo WPA::field($field, $elem_value, $option['id'], 'profile');
                }
            }

            echo '</div>';

            do_action('wpa_html_profile_after');
        }

        // save profile form fields
        public function save_profile($user_id)
        {
            if (wp_verify_nonce(wpa_get_var('wpa-profile-nonce'), 'wpa-profile')) {
                $errors  = array();
                $profile = wpa_get_var('profile');
                foreach ($this->sections as $request_value) {
                    if ($profile == $request_value['profile']) {
                        ...
                        $request = apply_filters('wpa_save_profile', $request, $request_key, $user_id);
                        if (empty($request)) {
                ...
                set_transient('wpa-profile-transient', $errors, 10);
            }
        }

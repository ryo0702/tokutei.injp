 ...
 *
 */
if (!class_exists('WPA_Field_sorter')) {
    class WPA_Field_sorter extends WPA_Fields
    {
        ...
        public function render()
        {
            ...
            $args = wp_parse_args($this->field, array(
                    'disabled'       => true,
                    'enabled_title'  => esc_html__('Enabled', 'nutrail'),
                    'disabled_title' => esc_html__('Disabled', 'nutrail'),
                )
            );
            ...
            echo '<div class="wpa-sorter" data-depend-id="'.esc_attr($this->field['id']).'"></div>';

            echo ($args['disabled']) ? '<div class="wpa-modules">' : '';

            echo (!empty($args['enabled_title'])) ? '<div class="wpa-sorter-title">'.esc_attr($args['enabled_title']).'</div>' : '';
            echo '<ul class="wpa-enabled">';
            ...
            if ($args['disabled']) {
                ...
                echo '<div class="wpa-modules">';
                echo (!empty($args['disabled_title'])) ? '<div class="wpa-sorter-title">'.esc_attr($args['disabled_title']).'</div>' : '';
                echo '<ul class="wpa-disabled">';

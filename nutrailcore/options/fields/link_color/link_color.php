 ...
 *
 */
if (!class_exists('WPA_Field_link_color')) {
    class WPA_Field_link_color extends WPA_Fields
    {
        ...
        public function render()
        {
            ...
            $color_props = array(
                ...
                'active'  => esc_html__('Active', 'nutrail'),
                'visited' => esc_html__('Visited', 'nutrail'),
                'focus'   => esc_html__('Focus', 'nutrail'),
            );
            ...
            foreach ($color_props as $color_prop_key => $color_prop_value) {
                if (!empty($args[$color_prop_key])) {
                    ...
                    echo '<div class="wpa--left wpa-field-color">';
                    echo '<div class="wpa--title">'.esc_attr($color_prop_value).'</div>';
                    echo '<input type="text" name="'.esc_attr($this->field_name('['.$color_prop_key.']')).'" value="'.esc_attr($value[$color_prop_key]).'" class="wpa-color"'.$default_attr.$this->field_attributes().'/>';
                    echo '</div>';

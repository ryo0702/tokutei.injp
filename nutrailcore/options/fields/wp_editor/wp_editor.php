 ...
 *
 */
if (!class_exists('WPA_Field_wp_editor')) {
    class WPA_Field_wp_editor extends WPA_Fields
    {
        ...
        public function render()
        {
            ...
            echo (wpa_wp_editor_api()) ? '<div class="wpa-wp-editor" data-editor-settings="'.esc_attr(json_encode($editor_settings)).'">' : '';

            echo '<textarea name="'.esc_attr($this->field_name()).'"'.$this->field_attributes($attributes).$editor_height.'>'.$this->value.'</textarea>';

            echo (wpa_wp_editor_api()) ? '</div>' : '';
        ...
        {
            if (wpa_wp_editor_api() && function_exists('wp_enqueue_editor')) {
                wp_enqueue_editor();
        ...
        public function setup_wp_editor_media_buttons()
        {
            ...
            echo 'var wpa_media_buttons = '.wp_json_encode($media_buttons).';';
            echo '</script>';
        }

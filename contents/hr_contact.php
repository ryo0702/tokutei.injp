<div class="result_contact">
    <div class="container_s">
        ...
        // Title
        echo '<div class="result_title"><h1>お見積りID '.@esc_attr($_GET['id']).' / '.$this_title.'のお見積り問い合わせ</h1></div>';
        ?>
            ...
            <form id="contact-form" action="" method="post" enctype="multipart/form-data">
                ...
                <ul class="form_list">
                    <li>
                        ...
                        <?php
                        $estimatedata = null;
                        $estimatedata = get_post_meta(@esc_attr($_GET['id']), 'estimates', true );
                        if(!empty($estimatedata['kanridantai_name'])){
                            echo '<p>見積もりID：'.@esc_attr($_GET['id']).' / '.@$estimatedata['kanridantai_name'].'</p>';
                            echo '<input type="hidden" name="kanridantai_name" value="'.$estimatedata['kanridantai_name'].'" />';
                        }
                        else{
                            echo '<p>見積もりID：'.@esc_attr($_GET['id']).' / 監理団体名非公開</p>';
                        ...
                        if(!empty($estimatedata['okuridashi'])){
                            $okuridashi_name = null;
                            $okuridashi_name = get_the_title($estimatedata['okuridashi']);
                            echo '<p>送り出し機関：'.@esc_attr($okuridashi_name).'</p>';
                            echo '<input type="hidden" name="okuridashi_id" value="'.$estimatedata['okuridashi'].'" />';
                            echo '<input type="hidden" name="okuridashi_name" value="'.$okuridashi_name.'" />';
                        }
                        else{
                            echo '<p>送り出し機関：送り出し機関未設定</p>';
                            echo '<input type="hidden" name="okuridashi_name" value="送り出し機関未設定" />';
                        }
                        ?>
                    </li>
                    <li>
                            ...
                            foreach ($array_shokushu_group as $group_title => $group_opt) {
                                echo '<optgroup label="'.$group_title.'">';
                                    foreach ($group_opt as $opt_value) {
                                        $selected = null;
                                        if(!empty($_GET['shokushu']) and $_GET['shokushu'] == $opt_value[0]){
                                            $selected = ' selected';
                                        }
                                        echo '<option value="'.$opt_value[0].'"'.$selected.'>'.$opt_value[1].'</option>';
                                    }
                                echo '</optgroup>';
                            }
                            ?>

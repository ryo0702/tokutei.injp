...
if(!empty($_POST['submit'])){
    ...
    // PageTitle
    if(!empty($_GET['nation']) and !empty($result_title_shokushu)){
        $page_title = $_GET['nation'].'対応の'.$result_title_shokushu.'の監理団体のお見積り';
    }
    elseif(!empty($_GET['nation'])){
        $page_title = $_GET['nation'].'の送り出し機関';
    }
    elseif(!empty($result_title_shokushu)){
        $page_title = $result_title_shokushu.'のお見積り';
    }
    else{
        $page_title = '送り出し機関 一覧';
    }
    // Page Description
    ...
    elseif(!empty($_GET['nation'])){
        $page_description = $_GET['nation'].'で外国人技能実習生を雇用をお考えならばやはり安い監理費の監理団体が良いとお考えの企業様に、最適な組合をご紹介する非公開お見積りを含めた監理団体お見積り比較システムです。';
    }
    elseif(!empty($result_title_shokushu)){
    ...
    if(!empty($_GET['shokushu']) or !empty($_GET['nation'])){
        $array_pankuzu = array(
            array(site_url(),'トップページ'),
            array(site_url('/?pagetype=okuridashi_result'),'送り出し一覧'),
    ...
    $pagination_url = site_url('/?pagetype=okuridashi_result');
    // if(!empty(@$_GET['nation'])){
    //     $args['meta_query'][] = array(
    //         'key' => 'okuridashi',
    //         'value' => serialize(esc_attr($_GET['nation'])),
    //         'compare' => 'Like'
    //     );
    //     $pagination_url .= '&nation='.$_GET['nation'];
    // }
    // if(!empty(@$_GET['shokushu'])){
    //     $args['meta_query'][] = array(
    //         'key' => 'okuridashi',
    ...
    include locate_template('contents/okuridashi_result.php');
    echo '</main>';
